    ...
    <div className="space-y-6">
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-6">
        <button
          ...
          className="group relative shrink-0 outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-[color:var(--profile-accent,#185FA5)] disabled:opacity-50"
          aria-label="Change profile photo"
        >
          <span
            className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border-4 border-white text-3xl font-semibold text-white shadow-md ring-2 ring-neutral-200/80 transition group-hover:ring-[color:var(--profile-accent,#185FA5)]"
            style={{ backgroundColor: photoUrl ? '#fff' : accent, color: photoUrl ? undefined : '#fff' }}
          >
            {photoUrl ? (
            ...
            return (
              <button
                ...
                className={`h-10 w-10 rounded-full border-2 shadow-sm transition hover:scale-105 disabled:opacity-50 ${
                  selected ? 'border-neutral-900 ring-2 ring-offset-2 ring-neutral-900' : 'border-white ring-1 ring-neutral-300'
                }`}
